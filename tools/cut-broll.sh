...
# surrounded by Reddit chrome. Each shot here is cropped to the game itself and
...
# Two source framings, measured from the footage:
#   fullscreen  -> the shaft sits at x 647..1234, full height
#   modal       -> the game sits at x 774..1147, y 266..861
# y=48 skips the dark Devvit title bar, so no black band bleeds into the frame.
...
[bg][fg]overlay=(W-w)/2:0,format=yuv420p" \
    -an -r 30 -c:v libx264 -preset medium -crf 18 "$OUT/$name.mp4"
  printf "  %-22s %5.1fs\n" "$name" "$(ffprobe -v error -show_entries format=duration -of csv=p=0 "$OUT/$name.mp4")"
}

echo "cutting shots..."

# 1. COLD OPEN: the marble runs the machine. (fullscreen, sharpest source)
...
# 3. The machine itself, as "nobody decides what stays" lands.
#    Taken from 44s, NOT 39s: at 39s a result card is still on screen, which both
#    duplicates the dissolve card one beat later and shows a card with no dissolve
#    line on it, stealing the punch from the real moment.
cut 03-machine        "$SRC/2026-07-14 08-43-57.mp4"          44.0 5.0  "$FULL"
...
# 6. THE RECORD RUN: descent into the record line, slow motion.
cut 06-record-run     "$SRC/2026-07-14 08-33-21.mp4"           8.3 7.5  "$MODAL"

# 7. NEW RECORD: confetti, and three real names on the board.
cut 07-new-record     "$SRC/2026-07-14 08-33-21.mp4"          15.3 4.0  "$MODAL"

# 8. THE CLOSE: the feed card, built by 3 redditors.
cut 08-feed-card      "$SRC/2026-07-14 08-54-18.mp4"         113.5 7.0  "$MODAL"
