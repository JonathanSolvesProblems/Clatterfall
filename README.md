...
- **Physics:** `matter-js` (the same engine Phaser ships as `Phaser.Physics.Matter`) runs headless in the Devvit Node runtime. A 200-part machine, about 400 bodies and roughly what a full season looks like, simulates in ~6ms. That number is measured and asserted by a perf test, not estimated.
